...
DECLARE
  ...
  v_session_date_value date;
  v_is_new_client boolean := false;
BEGIN
  -- Convert session date
  v_session_date_value := p_session_date::date;
  
  -- Get class capacity
  SELECT c.capacity INTO v_class_capacity
  FROM class_sessions cs
  JOIN classes c ON c.id = cs.class_id
  WHERE cs.id = p_class_session_id AND cs.studio_id = p_studio_id;
  ...
  AND session_date = v_session_date_value
  ...
  FROM clients
  WHERE studio_id = p_studio_id AND email = p_client_email;
  
  IF v_client_id IS NULL THEN
    -- Create new client
    INSERT INTO clients (studio_id, email, name, phone, total_visits)
    VALUES (p_studio_id, p_client_email, p_client_name, p_client_phone, 0)
    RETURNING id INTO v_client_id;
    
    v_is_new_client := true;
  ELSE
    -- Update existing client info if provided details are more complete
    UPDATE clients 
    SET 
      name = CASE WHEN p_client_name IS NOT NULL AND p_client_name != '' THEN p_client_name ELSE name END,
      phone = CASE WHEN p_client_phone IS NOT NULL AND p_client_phone != '' THEN p_client_phone ELSE phone END,
      updated_at = NOW()
  ...
  SET 
    ...
    last_visit_date = GREATEST(last_visit_date, v_session_date_value::timestamptz),
    updated_at = NOW()
  WHERE id = v_client_id;
  
  RETURN QUERY SELECT v_booking_id, v_client_id, v_is_new_client;
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE bookings remove COLUMN client_name;
ALTER TABLE bookings remove COLUMN client_email;
ALTER TABLE bookings remove COLUMN client_phone;
